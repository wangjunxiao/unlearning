...
def calculate_cdp(features:dict, classes:list, dataset:str, coe:int):
    features_class_wise = {}
    tf_idf_map = {}
    for i in features:
        '''Class-wise Activation'''
        calc_class_wise_feature(features[i], i, classes, dataset, features_class_wise=features_class_wise)
        #calc_tf_idf(features_class_wise[i], i, coe=coe, tf_idf_map=tf_idf_map)
        break #test###########################
    return tf_idf_map
        
def calc_class_wise_feature(feature:dict, name:str, classes:list, dataset:str, features_class_wise:dict):
    if dataset == 'cifar10':
        class_num = 10
        print(feature.shape, len(classes))
        features_class_wise[name] = 1

                                                # c - filters; n - classes
def calc_tf_idf(feature:dict, name:str, coe:int, tf_idf_map:dict):   # feature = [c, n] ([64, 50000])    
    # calc tf
