...
from FL_base import fedavg, global_train_once, FL_Train
...
def unlearning(old_GMs, old_CMs, client_data_loaders, test_loader, FL_params):
    ...
    if(not(FL_params.forget_client_idx in range(FL_params.N_client))):
        raise ValueError('FL_params.forget_client_idx is not assined correctly, forget_client_idx should in {}'.format(range(FL_params.N_client)))
