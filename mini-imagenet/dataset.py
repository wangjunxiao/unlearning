...
def read_csv_classes(csv_dir: str, csv_name: str):
    data = pd.read_csv((csv_dir / csv_name))
...
def calculate_split_info(path: str, label_dict: dict, rate: float = 0.2):
    ...
    for label in labels:
        ...
        if imshow_flag:
            ...
            img = Image.open(path / 'images' / img_label / img_name)
            plt.imshow(img)
    ...
    #print(new_val_data)
    #print(new_val_data.iloc[1].values)

    # save new csv data
    new_train_data.to_csv((path / "new_train.csv"))
    new_val_data.to_csv((path / "new_val.csv"))


def main():
    
    project_dir = Path(__file__).resolve().parent.parent
    dataset_dir = project_dir / 'data' /'mini-imagenet' 
    json_path = dataset_dir / 'label.json'

    # load imagenet labels
    label_dict = json.load(open(json_path, "r"))
    label_dict = dict([(v[0], v[1]) for k, v in label_dict.items()])

    calculate_split_info(dataset_dir, label_dict)
