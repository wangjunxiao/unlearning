...
def train(model, loader, epoch, optimizer, criterion, device, dtype, batch_size, log_interval, scheduler, warm_scheduler=None):
    ...
    for batch_idx, (data, target) in enumerate(loader):
        ...
        if_warmup=False if warm_scheduler==None else warm_scheduler.if_in_warmup()
        
        if isinstance(scheduler, CyclicLR) and not if_warmup:
        ...
        if batch_idx % log_interval == 0:
            print(
                ...
                'lr: {:.6f}.'.format(epoch, batch_idx, len(loader),
