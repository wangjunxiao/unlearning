...
parser.add_argument('--unlearn_class', type=str,
...
def load_model_CIFAR10(args, model_path):
    if args.model == 'resnet56':
        ...
        net = VGG_CIFAR(cfg_index=11, num_classes=10)
    elif args.model == 'vgg19':
        net = VGG_CIFAR(cfg_index=19, num_classes=10)
    else:
        print('no model')
        return
    net = net.cuda()
...
def Class_Pruning():
    ...
    args.warmup_step = 0
    args.warm_lr = 10e-5
    args.model_file = 'seed0_acc84.06_epoch10_2021-10-12 12-13-52.pth'
    print(args)
    
    setup_seed(args.seed)
    model_path = project_dir / 'ckpt' / args.model / args.model_file
    multi_path = 'multi'+args.unlearn_class
    pruned_save_info = project_dir / 'ckpt' / 'pruned' / args.model / multi_path
    finetuned_save_info = project_dir / 'ckpt' / 'finetuned' / args.model / multi_path
    ...
    unlearn_listclass = args.unlearn_class.split('+')
    unlearn_listclass = list(map(int, unlearn_listclass))
    unlearn_testdata = generate(testset, unlearn_listclass)
    for unlearn_class in unlearn_listclass:
    ...
    for unlearn_class in unlearn_listclass:
        ...
        tf_idf_map = calculate_cp(feature_iit, classes, args.dataset, args.coe, unlearn_class=unlearn_class)
        ...
        pruned_model_path = pruned_save_info / ('seed_'+str(args.seed)+
                                            ...
                                            '_model_class_'+ str(unlearn_class) +'.pth')
        pruned_mask_path = pruned_save_info / ('seed_'+str(args.seed)+
                                           time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())+
                                            '_mask_class_'+ str(unlearn_class) +'.pth')
        pruner.export_model(pruned_model_path, pruned_mask_path)
        net = load_model_CIFAR10(args, pruned_model_path)
    
        '''test after each class pruning'''
        print('*'*5+'testing in unlearn_data'+'*'*12)
        test(net, unlearn_testloader)
        print('*'*40)
        print('*'*5+'testing in rest_data'+'*'*15)
